...
def nmtoken_from_string(text):
    """
    Return a Nmtoken from a string.
...
def escape_javascript_string(text, escape_for_html=True,
                             escape_quote_for_html=False, escape_CDATA=True,
                             escape_script_tag_with_quote='"'):
    r"""
    Escape text in order to be used as Javascript string in various context.
    ...
    .. doctest::
        ...
        >>> text = '''foo</script>bar'''
        >>> val = escape_javascript_string(text, escape_for_html=False,
        ...                                escape_script_tag_with_quote='"')
        >>> mycode = '''alert("%s")''' % val
        'alert("foo</scr"+"ipt>bar")'

    C{escape_script_tag_with_quote} is not considered when
    C{escape_for_html} is set to True.

    If you are planning to return the string as part of a pure Javascript
    document, then you should in principle set both C{escape_for_html} and
    C{escape_CDATA} to False, and C{escape_script_tag_with_quote} to None.

    :param text: string to be escaped
    :param escape_for_html: if True, also escape input for HTML
    :param escape_CDATA: if True, escape closing CDATA tags (when
        C{escape_for_html} is False) :escape_script_tag_with_quote: which quote
        will be used to delimit your string, in case you must wash, but keep,
        C{</script>} tag (when C{escape_for_html} is False)
    ...
    if not escape_for_html and escape_script_tag_with_quote:
        text = RE_CLOSING_SCRIPT_TAG.sub(
            '''</scr%(q)s+%(q)sipt>''' %
            {'q': escape_script_tag_with_quote}, text)
...
class HTMLWasher(HTMLParser):
    ...
    """
    Create a washer for HTML, avoiding XSS attacks.

    See wash function for details on parameters.
    ...
    def wash(self, html_buffer,
             ...
             allowed_attribute_whitelist=None):
        """
        ...
        self.allowed_attribute_whitelist = allowed_attribute_whitelist or \
            CFG_HTML_BUFFER_ALLOWED_ATTRIBUTE_WHITELIST
    ...
    def handle_attribute_value(self, value):
        ...
        Especially designed for avoiding URLs in the form:
    ...
    def handle_charref(self, name):
        """Process character references of the form "&#ref;".
...
def tidy_html(html_buffer, cleaning_lib='utidylib'):
    ...
    :note: requires uTidylib or BeautifulSoup to be installed. If the chosen
    library is missing, the input X{html_buffer} is returned I{as is}.
...
def get_mathjax_header(https=False):
    """
    Return the snippet of HTML code to put in HTML HEAD tag.
...
<script src="%(mathjax_path)s/MathJax.js?config=%(mathjax_config)s"
...
def is_html_text_editor_installed():
    """Return True if the wysiwyg editor (CKeditor) is installed."""
    return os.path.exists(os.path.join(cfg['CFG_WEBDIR'], 'ckeditor',
                                       'ckeditor.js'))

ckeditor_available = LocalProxy(is_html_text_editor_installed)


def get_html_text_editor(name, id=None, content='', textual_content=None,
                         width='300px', height='200px',
                         enabled=True, file_upload_url=None,
                         toolbar_set="Basic",
                         custom_configurations_path='/js/ckeditor/invenio-cked'
                                                    'itor-config.js',
                         ln=None):
    """
    Return a wysiwyg editor (CKEditor) to embed in html pages.
...
/* Load the script only once, or else multiple instance of the editor on the
   same page will not work */
var INVENIO_CKEDITOR_ALREADY_LOADED
if (INVENIO_CKEDITOR_ALREADY_LOADED != 1) {
    document.write('<scr'+'ipt type="text/javascript" '+
                   'src="%(CFG_SITE_URL)s/vendors/ckeditor/ckeditor.js">'+
                   '<\/script>');
    INVENIO_CKEDITOR_ALREADY_LOADED = 1;
}
//]]></script>
<input type="hidden" name="editor_type" id="%(id)seditortype"
       value="textarea" />
<textarea rows="100" cols="80" id="%(id)s" name="%(name)s"
          style="width:%(width)s;height:%(height)s"
>%(textual_content)s</textarea>
<textarea rows="100" cols="80" id="%(id)shtmlvalue" name="%(name)shtmlvalue"
          style="display:none;width:%(width)s;height:%(height)s"
>%(html_content)s</textarea>
...
CKEDITOR.on('instanceReady',
  function( evt )
  {
    /* If CKeditor was correctly loaded, display the nice HTML
       representation */
...
//]]></script>''' % {
        ...
        textarea = '<textarea rows="100" cols="80" %(id)s name="%(name)s" ' \
            'style="width:%(width)s;height:%(height)s">%(content)s' \
            '</textarea>' % {
...
def unescape(s, quote=False):
    ...
    Replace escaped characters '&amp;', '&lt;' and '&gt;' with the
    corresponding regular characters. If the optional flag quote is true, the
    escaped quotation mark character ('&quot;') is also translated.
    """
    s = s.replace('&lt;', '<')
    s = s.replace('&gt;', '>')
    if quote:
...
class EscapedString(str):
    ...
    """
    Stub used by the MLClass machinery.

    It distinguishes native string, from string that don't need to be escaped.
    """
...
class EscapedHTMLString(EscapedString):
    ...
    """
    Automatically escape a non-escaped string used to initialize it.

    It uses the HTML escaping method (i.e. cgi.escape).
    """
    ...
    def __new__(cls, original_string='', escape_quotes=False):
        ...
        if isinstance(original_string, EscapedString):
            ...
            if original_string and not str(original_string).strip():
                ...
                escaped_string = cgi.escape(str(original_string),
                                            escape_quotes)
    ...
    def __repr__(self):
        ...
        return 'EscapedHTMLString(%s, %s)' % (repr(self.original_string),
                                              repr(self.escape_quotes))
...
class EscapedXMLString(EscapedString):
    ...
    It uses the XML escaping method (i.e. encode_for_xml).
    """
    ...
    def __repr__(self):
        """Get string representation."""
        return 'EscapedXMLString(%s, %s)' % (repr(self.original_string),
                                             repr(self.escape_quotes))
...
def create_tag(tag, escaper=EscapedHTMLString, opening_only=False, body=None,
               escape_body=False, escape_attr=True, indent=0, attrs=None,
               **other_attrs):
    """
...
class MLClass(object):
    ...
    def __getattr__(self, tag):
        """TODO."""
        def tag_creator(body=None, opening_only=False, escape_body=False,
                        escape_attr=True, indent=0, attrs=None, **other_attrs):
            if body:
                return create_tag(tag, body=body, opening_only=opening_only,
                                  escape_body=escape_body,
                                  escape_attr=escape_attr, indent=indent,
                ...
                def handle_body(*other_bodies):
                    ...
                    if other_bodies:
                        ...
                        for body in other_bodies:
                            if callable(body) and \
                                    body.__name__ == 'handle_body':
                                full_body += body()
                            elif isinstance(body, EscapedString):
                                full_body += body
                            else:
                                full_body += self.escaper(str(body))
                    return create_tag(tag, body=full_body,
...
def create_html_select(options, name=None, selected=None, disabled=None,
                       multiple=False, attrs=None, **other_attrs):
    """
    Create an HTML select box.

    .. doctest::

        >>> print(create_html_select(["foo", "bar"], selected="bar",
        ...                          name="baz"))
        <select name="baz">
        ...
        >>> print(create_html_select([("foo", "oof"), ("bar", "rab")],
        ...                          selected="bar", name="baz"))
        <select name="baz">
          <option value="foo">
            oof
          </option>
    ...
    if isinstance(options, dict):
        ...
        for item in options:
            if isinstance(item, (str, unicode)):
                ...
                raise ValueError('Item "%s" of incompatible type: %s' %
                                 (item, type(item)))
    else:
    ...
    for key, value in items:
        ...
        body.append(create_tag("option", body=value, escape_body=True,
                               value=key, attrs=option_attrs))
    if attrs is None:
        attrs = {}
    if name is not None:
        attrs['name'] = name
...
class _LinkGetter(HTMLParser):
    ...
    """
    Intercept the a tags and retrieve the href attribute.
