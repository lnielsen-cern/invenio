...
class XmpValidation(object):
    ...
    def __get_title_and_authors(self):
        if self.metadata:
            ...
            if ('xmp_title' in self.metadata) and \
               ('xmp_creator' in self.metadata):
                if self.metadata['xmp_title']:
                    title = self.metadata['xmp_title']
                ...
                if (self.metadata['xmp_creator']) and \
                   (len(self.metadata['xmp_creator']) > 0):
                    authors = self.metadata['xmp_creator']
